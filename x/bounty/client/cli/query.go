...
Example:
$ %s query bounty programs --creator cosmos1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			creatorAddr, _ := cmd.Flags().GetString(FlagCreatorAddress)

			if len(creatorAddr) != 0 {
				_ = sdk.MustAccAddressFromBech32(creatorAddr)
			}
	...
	cmd.Flags().String(FlagCreatorAddress, "", "(optional) filter by programs created by creator")
